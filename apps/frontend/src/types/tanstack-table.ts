...
type CellMeta<TData extends RowData, TValue> = Meta & {
  onClick?: (event: Parameters<NonNullable<ComponentProps<"td">["onClick"]>>[0], context: CellContext<TData, TValue>) => void;
};

type HeadMeta<TData extends RowData, TValue> = Meta & {
  onClick?: (event: Parameters<NonNullable<ComponentProps<"th">["onClick"]>>[0], context: HeaderContext<TData, TValue>) => void;
};
